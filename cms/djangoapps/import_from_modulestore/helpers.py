...
def _process_import(usage_key_string, block_to_import) -> list[PublishableVersionWithMapping]:
    ...
    if block_to_import.tag not in CompositionLevel.OLX_COMPLEX_LEVELS.value:
        return _import_simple_block(block_to_import, usage_key)

    for child in block_to_import.getchildren():
        child_usage_key_string = get_usage_key_string_from_staged_content(
            staged_content, child.get('url_name')
        )
        if not child_usage_key_string:
            continue

        result.extend(_import_child_block(child, child_usage_key_string))

    if composition_level == CompositionLevel.COMPONENT.value:
        return [
...
def _import_simple_block(block_to_import, usage_key) -> list[PublishableVersionWithMapping]:
    """
    ...
    """
    publishable_version_with_mapping = _create_block_in_library(block_to_import, usage_key)
    return [publishable_version_with_mapping] if publishable_version_with_mapping else []

def _import_child_block(child, child_usage_key_string):
    """
    ...
    if child.tag in CompositionLevel.OLX_COMPLEX_LEVELS.value:
        return _import_complicated_child(child, child_usage_key_string)
    else:
        return _import_simple_block(child, child_usage_key)
        
def _import_complicated_child(child, child_usage_key_string):
    """
    ...
    """
    child_component_versions_with_mapping = _process_import(child_usage_key_string, child)
    try:
        container_type = ContainerType(child.tag)
    except ValueError:
        container_type = None
...
def _create_block_in_library(self, block_to_import, usage_key) -> PublishableVersionWithMapping | None:
    ...
    with transaction.atomic():
        ...
        publishable_entity_mapping, _ = get_or_create_publishable_entity_mapping(
            usage_key,
            component_version.component.publishable_entity,
        )
...
def get_or_create_publishable_entity_mapping(usage_key, entity: PublishableEntity) -> tuple[PublishableEntityMapping, bool]:
    """
    ...
    return PublishableEntityMapping.objects.get_or_create(
        ...
        target_package=entity.learning_package,
    )
...
def _get_node_for_usage_key(node: etree._Element, usage_key: UsageKey) -> etree._Element:
    """
    ...
    for child in node.getchildren():
        found = _get_node_for_usage_key(child, usage_key)
